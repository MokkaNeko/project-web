...
<body>
    ...
    if (isset($_POST['delete'])) {
        ...
        $lihatStmt = $conn->prepare("SELECT * from jadwal_pelajaran WHERE id = ? AND hari = ?");
        ...
        if ($lihatResult->num_rows > 0) {
            ...
            $stmt = $conn->prepare("UPDATE jadwal_pelajaran SET mapel = ?, guru = ?, waktu = ? WHERE id = ?");
            $stmt->bind_param("sssi", $mapel, $guru, $waktu, $id);
    ...
    $stmt = $conn->prepare("SELECT * from jadwal_pelajaran WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    ?>
        ...
        <form method="POST" action="">
            ...
            <input type="text" name="guru" value="<?php echo $row['guru']; ?>"><br>
